...
class DeviceResidentBatcher:
    ...
    The whole dataset already fits on the GPU/MPS device at once for this
    project's data sizes, so moving eeg/nirs to the device ONCE here and
    slicing batches in-place avoids a host->device transfer on every single
    batch -- the dominant per-step overhead once data loading itself isn't
    the bottleneck (see DataLoader usage elsewhere in this file, which is
    correct for larger-than-memory datasets but pays this transfer cost
    needlessly here).
    """
...
def pretrain_one_fold(model, train_loader, device,
    ...
    for epoch in epoch_bar:
        ...
        for eeg_b, nirs_b, _ in train_loader:
            # eeg_b/nirs_b are already on-device (DeviceResidentBatcher) -- no per-batch transfer
            optimizer.zero_grad()
...
def extract_embeddings(model, loader, device):
    ...
    with torch.no_grad():
        for eeg_b, nirs_b, lbl_b in loader:
            # eeg_b/nirs_b are already on-device (DeviceResidentBatcher)
