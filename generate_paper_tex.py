...
\item \textbf{Modality ablation context.} Comparing our fNIRS-only / EEG-only / fNIRS+EEG rows against Arif et al.'s Table~4 lets us check whether the multimodal advantage they report (fNIRS+EEG $>$ either alone) holds at our smaller subject count, or whether it is an artifact of their larger 26-subject, 20/6 split.
...
def main():
    ...
    tex = tex.replace("{{FIG_EEG_ERP}}", figure_block(
        "shin2018_fig2_eeg_erp.jpg",
        "EEG event-related potentials, topographic maps, and time-frequency analysis "
        "for the n-back task (Dataset A). Reproduced from Shin et al.\\ 2018, "
        "\\textit{Scientific Data} (CC-BY 4.0).",
        "fig:eeg_erp", width="0.85"))
    tex = tex.replace("{{FIG_NIRS_HRF}}", figure_block(
        "shin2018_fig3_nirs_hrf.jpg",
        "fNIRS hemodynamic response and topographic maps across 0-/2-/3-back "
        "difficulty (Dataset A). Reproduced from Shin et al.\\ 2018, "
        "\\textit{Scientific Data} (CC-BY 4.0).",
        "fig:nirs_hrf", width="0.85"))
    ...
    tex = tex.replace("{{FIG_EDA}}", "\n\n".join([
        figure_block("VP001_eeg_raw.png",
                     "Raw EEG, subject VP001, first 30~s, first 5 channels.",
                     "fig:eda", width="0.85"),
        figure_block("VP001_nirs_raw.png",
                     "Raw fNIRS HbO/HbR, subject VP001, first 60~s, first 2 channels.",
                     "fig:eda_nirs", width="0.85"),
