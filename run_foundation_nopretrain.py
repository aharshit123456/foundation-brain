...
def run_nopretrain_loso(all_data, batch_size=64):
    ...
    for test_subj in fold_bar:
        ...
        # Z-score normalize using train-fold statistics only -- see run_efnet_loso.py for
        # the original diagnosis (NIRS ~1e-3 scale vs EEG ~1e1 scale). Matters less here
        # since random weights don't suffer vanishing gradients, but the linear probe on
        # top still benefits from well-scaled inputs producing more informative features,
        # and consistency with the pretrained-model comparison requires it regardless.
